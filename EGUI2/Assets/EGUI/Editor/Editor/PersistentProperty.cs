...
namespace EGUI.Editor
{
    public class PersistentProperty
    {
        ...
        {
            get { return mMemberPath[mMemberPath.Length - 1]; }
        }

        public string displayName
        {
            get { return UserUtil.GetNiceDisplayName(memberInfo.Name); }
        }
        ...
        public int length
        {
            get { return persistentObject.values.Length; }
        }

        public bool exist
        {
            get { return GetValues<object>().All(i => i != null); }
        }
        ...
        public void ResizeArray(int len)
        {
            ...
            {
                if (mi.MemberType == MemberTypes.Field)
                    Debug.Assert(!((FieldInfo) mi).IsInitOnly);
                else if (mi.MemberType == MemberTypes.Property)
                    Debug.Assert(((PropertyInfo) mi).CanWrite);
            }
            ...
            for (var i = 0; i < arrays.Length; i++)
            {
                ...
                if (array.Length != len)
                {
                    var newArray = Array.CreateInstance(type.GetElementType(), len);
                    Array.ConstrainedCopy(array, 0, newArray, 0, Mathf.Min(array.Length, len));
